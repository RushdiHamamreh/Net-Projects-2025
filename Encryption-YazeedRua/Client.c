...
// MUST match server key (32 bytes for AES-256)
...
int main(int argc, char* argv[]) {
    ...
    if (file_size > 100000000LL) { // 100 MB limit
        fclose(fp);
        printf("[-] File too large (>100 MB)\n");
        return 1;
    }
    ...
    unsigned char file_size_bytes[8] = {
        ((unsigned long long)file_size >> 56) & 0xFF,
        ((unsigned long long)file_size >> 48) & 0xFF,
        ((unsigned long long)file_size >> 40) & 0xFF,
        ((unsigned long long)file_size >> 32) & 0xFF,
        ((unsigned long long)file_size >> 24) & 0xFF,
        ((unsigned long long)file_size >> 16) & 0xFF,
        ((unsigned long long)file_size >> 8) & 0xFF,
        (unsigned long long)file_size & 0xFF
    };
    if (send_all(sock, file_size_bytes, 8) < 0 ||
        send_all(sock, file_data, (int)file_size) < 0) {
        printf("[-] Failed to send file data\n");
        goto cleanup;
    }

    printf("[+] Sent file: %s (%lld bytes)\n", filename, file_size);
    ...
    unsigned char* encrypted_data = malloc((size_t)enc_size);
    if (!encrypted_data) {
        printf("[-] Memory allocation failed\n");
        goto cleanup;
    }
    if (recv_all(sock, encrypted_data, (int)enc_size) < 0) {
    ...
    if (ef) {
        fwrite(encrypted_data, 1, (size_t)enc_size, ef);
        fclose(ef);
